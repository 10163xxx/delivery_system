          ...
          <p className="meta-line" style={{ marginTop: '16px' }}>
            ...
            <strong>
              {rechargeAmountPreview !== null
                ? formatPrice(Math.round(rechargeAmountPreview * CURRENCY_CENTS_SCALE))
                : '--'}
            </strong>
            ，充值后余额预计为{' '}
            <strong>
              {rechargeAmountPreview !== null
                ? formatPrice(
                    selectedCustomer.balanceCents +
                      Math.round(rechargeAmountPreview * CURRENCY_CENTS_SCALE),
                  )
                : '--'}
            </strong>
            。
...
function CouponsPanel({ props }: { props: CustomerRoleProps }) {
  const { selectedCustomer, formatPrice, formatTime, navigate } = props
        ...
        <>
          ...
          <div className="ticket-grid">
            {selectedCustomer.coupons.length === 0 ? (
              ...
              selectedCustomer.coupons.map((coupon: Coupon) => (
...
function AddressesPanel({ props }: { props: CustomerRoleProps }) {
  const {
    selectedCustomer,
    navigate,
    addressFormErrors,
...
function ProfileOverviewPanel({ props }: { props: CustomerRoleProps }) {
  const { selectedCustomer, formatPrice, openRechargePage, navigate } = props
            ...
            <div>
              ...
              <strong>
                {selectedCustomer.membershipTier === 'Member'
                  ? `${MEMBER_AUTO_DISPATCH_MINUTES} 分钟`
                  : `${STANDARD_AUTO_DISPATCH_MINUTES} 分钟`}
              </strong>
            </div>
...
export function CustomerProfilePanels(props: CustomerRoleProps) {
  if (props.customerWorkspaceView === 'recharge') {
    return <RechargePanel props={props} />
  }

  if (props.customerWorkspaceView === 'coupons') {
    return <CouponsPanel props={props} />
  }

  if (props.customerWorkspaceView === 'addresses') {
    return <AddressesPanel props={props} />
  }

  return <ProfileOverviewPanel props={props} />
}
