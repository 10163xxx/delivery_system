...
const ACTIVE_MERCHANT_ORDER_STATUSES = new Set<string>([
...
function getSortedOrders(orders: OrderSummary[]) {
  return orders.slice().sort((left, right) => {
    const activeOrderRank =
      Number(ACTIVE_MERCHANT_ORDER_STATUSES.has(right.status)) -
      Number(ACTIVE_MERCHANT_ORDER_STATUSES.has(left.status))
...
export function MerchantOrdersWorkspacePanel(props: MerchantConsolePanelProps) {
  ...
  const storeIdSet = useMemo(
    () => new Set(merchantStores.map((store) => store.id)),
    [merchantStores],
  )
  const orders = useMemo(
    () => getSortedOrders((state?.orders ?? []).filter((order) => storeIdSet.has(order.storeId))),
    [state?.orders, storeIdSet],
  )
