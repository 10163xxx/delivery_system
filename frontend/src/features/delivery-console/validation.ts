import type { MerchantDraft, MerchantFormField, MenuItemDraft, MenuItemFormField, CustomerAddressDraft, CustomerAddressField, MerchantProfileDraft, MerchantProfileFormField } from './types'
import {
...
import { buildCustomerAddressPayload, buildMenuItemPayload } from './payloads'
import { validateBusinessHours } from './schedule'
import { normalizeTextInput } from './shared'
  ...
  return {
    contactPhone:
      !contactPhone
        ...
        : new RegExp(`^[0-9+\\- ]{${MIN_PHONE_LENGTH},${MAX_CONTACT_PHONE_LENGTH}}$`).test(contactPhone)
          ? undefined
          : DELIVERY_CONSOLE_MESSAGES.contactPhoneInvalid,
    bankName: draft.payoutAccountType === 'bank' ? (bankName ? undefined : DELIVERY_CONSOLE_MESSAGES.bankNameRequired) : undefined,
