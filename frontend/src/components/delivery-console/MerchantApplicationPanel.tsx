...
export function MerchantApplicationPanel(props: any) {
  const {
    ...
    <Panel title="商家入驻申请" description="商家提交店铺资料后，需要管理员审核通过才能正式入驻。">
      ...
      {merchantApplicationView === 'pending' ? (
        <div className="ticket-grid">
          {merchantPendingApplications.length === 0 ? <div className="empty-card">当前没有待审核申请。</div> : merchantPendingApplications.map((application: any) => (
        ...
        <div className="ticket-grid">
          {merchantReviewedApplications.length === 0 ? <div className="empty-card">当前没有已审核申请。</div> : merchantReviewedApplications.map((application: any) => (
            <article key={application.id} className="ticket-card">
              <div className="ticket-header">
                <div><p className="ticket-kind">审核记录</p><h3>{application.storeName}</h3></div>
                <span className={application.status === 'Approved' ? 'badge success' : 'badge warning'}>{application.status === 'Approved' ? '已通过' : '已驳回'}</span>
              </div>
              <p>商家 {application.merchantName} · {application.category} · 预计出餐 {application.avgPrepMinutes} 分钟</p>
              <p className="meta-line">营业时间 {formatBusinessHours(application.businessHours)}</p>
        ...
        <>
          <div className="form-grid">
            ...
            <label className="full">
              <span>补充说明</span>
              <input value={merchantDraft.note} onChange={(event) => setMerchantDraft((current: any) => ({ ...current, note: event.target.value }))} />
            </label>
          </div>
