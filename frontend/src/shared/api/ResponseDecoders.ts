...
import type { AuthSession } from '@/auth/object/AuthSession'
...
import type { ImageUploadResponse } from '@/merchant/object/menu/ImageUploadResponse'
...
import type { OrderPartialRefundRequest, OrderPartialRefundResolution } from '@/order/object/after-sales/OrderPartialRefundRequest'
...
import {
  ...
  TICKET_KIND,
  TICKET_STATUS,
} from '@/shared/object/domain/DomainConstants'
import type { DeliveryAppState } from '@/shared/object/domain/DomainState'
...
const decodeSelectionGroup: Decoder<MenuItemSelectionGroup> = (value, path = '$') => ({ name: decodeField(value, path, 'name', decodeString), minSelections: decodeField(value, path, 'minSelections', decodeNumber), maxSelections: decodeField(value, path, 'maxSelections', decodeNumber), options: decodeField(value, path, 'options', (entry, entryPath) => decodeArray(entry, entryPath, decodeString)) })
...
const decodeStore: Decoder<Store> = (value, path = '$') => {
  const operations: StoreOperations = { status: decodeField(value, path, 'status', decodeStoreStatus), businessHours: decodeField(value, path, 'businessHours', decodeBusinessHours), avgPrepMinutes: decodeField(value, path, 'avgPrepMinutes', decodeNumber), imageUrl: decodeOptionalField(value, path, 'imageUrl', decodeString), menu: decodeField(value, path, 'menu', (entry, entryPath) => decodeArray(entry, entryPath, decodeMenuItem)) }
...
const decodeMerchantApplication: Decoder<MerchantApplication> = (value, path = '$') => {
  ...
  return { id: decodeField(value, path, 'id', decodeString), merchantName: decodeField(value, path, 'merchantName', decodeString), storeName: decodeField(value, path, 'storeName', decodeString), category: decodeField(value, path, 'category', decodeStoreCategory), businessHours: decodeField(value, path, 'businessHours', decodeBusinessHours), avgPrepMinutes: decodeField(value, path, 'avgPrepMinutes', decodeNumber), imageUrl: decodeOptionalField(value, path, 'imageUrl', decodeString), note: decodeOptionalField(value, path, 'note', decodeString), ...review, review }
}
...
export const decodeAuthUser: Decoder<AuthUser> = (value, path = '$') => ({ id: decodeField(value, path, 'id', decodeString), username: decodeField(value, path, 'username', decodeString), role: decodeField(value, path, 'role', decodeRole), displayName: decodeField(value, path, 'displayName', decodeString), linkedProfileId: decodeOptionalField(value, path, 'linkedProfileId', decodeString) })
export const decodeAuthSession: Decoder<AuthSession> = (value, path = '$') => ({ token: decodeField(value, path, 'token', decodeString), user: decodeField(value, path, 'user', decodeAuthUser) })
