...
object NoteStatus:
  private val DraftValue = "draft"
  private val PublishedValue = "published"
  val DatabaseValues: List[String] = List(DraftValue, PublishedValue)

  def toDatabase(status: NoteStatus): String =
    status match
      case NoteStatus.Draft => DraftValue
      case NoteStatus.Published => PublishedValue

  def fromDatabase(value: String): Either[String, NoteStatus] =
    value.trim.toLowerCase match
      case DraftValue => Right(NoteStatus.Draft)
      case PublishedValue => Right(NoteStatus.Published)
