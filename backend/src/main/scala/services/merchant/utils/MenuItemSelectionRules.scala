...
// Business note: menu item selection validation and additional-price calculation rules.
import system.app.*

import system.objects.given

import services.merchant.objects.*
import services.order.objects.OrderItemSelection
  ...
  options.flatMap(option =>
    sanitizeOptionalText(Some(option.name), DeliveryValidationDefaults.MenuItemSelectionOptionMaxLength).map(
      sanitizedName =>
        option.copy(
          name = sanitizedName,
          additionalPriceCents = maxCurrencyCents(NumericDefaults.ZeroCurrencyCents, option.additionalPriceCents),
        )
    )
  )
