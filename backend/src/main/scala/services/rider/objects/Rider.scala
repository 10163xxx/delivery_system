...
import services.merchant.objects.{MerchantPayoutAccount, MerchantWithdrawal, legacyMerchantPayoutAccountFromText}
    ...
    for
      ...
      availableToWithdrawCents <-
        cursor.getOrElse[CurrencyCents]("availableToWithdrawCents")(
          new CurrencyCents(Math.max(NumericDefaults.ZeroCurrencyCents, earningsCents - withdrawnCents))
        )
