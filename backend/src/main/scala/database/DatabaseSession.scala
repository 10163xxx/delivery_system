...
def withTransactionConnectionBlocking[A](operation: Connection => A): A =
  val dataSource =
    databaseDataSourceRef.get().getOrElse(
      throw new IllegalStateException(DatabaseRuntimeDefaults.NotInitializedMessage.raw)
    )
