...
private val insertEntityRowSqlByTable = entityTables.map(table =>
  table.tableName.raw -> new SqlStatement(s"""
...
private val deleteEntityRowsSqlByTable = entityTables.map(table =>
  table.tableName.raw -> new SqlStatement(s"delete from ${table.tableName.raw}")
).toMap
  ...
  if tableExists(connection, DeliveryPersistenceDefaults.SnapshotTableName) then
    IO.blocking {
      ...
      try
        ...
        try
          if resultSet.next() then Some(decodeJsonRow[DeliveryAppState](resultSet.getString(1), DeliveryPersistenceDefaults.SnapshotTableName))
          else None
...
private def loadEntityRows[A: Decoder](
  ...
  IO.blocking {
    val statement = connection.prepareStatement(loadEntityRowsSqlByTable(table.tableName.raw).raw)
    try
      val resultSet = statement.executeQuery()
      try
        Iterator
          ...
          .takeWhile(identity)
          .map(_ => decodeJsonRow[A](resultSet.getString(1), table.tableName))
          .toList
      finally resultSet.close()
    finally statement.close()
  }
...
private def replaceEntityRows[A: Encoder](
  ...
  executeBlockingSqlStatement(connection, deleteEntityRowsSqlByTable(table.tableName.raw))

  val insertStatement = connection.prepareStatement(insertEntityRowSqlByTable(table.tableName.raw).raw)
  try
    values.zipWithIndex.foreach { case (value, index) =>
      insertStatement.setString(1, entityIdOf(value))
      insertStatement.setInt(2, index)
      insertStatement.setString(3, deliveryStateJsonPrinter.print(value.asJson))
...
private def loadMetricsRow(connection: Connection): IO[Option[SystemMetrics]] =
  IO.blocking {
    ...
    try
      ...
      val resultSet = statement.executeQuery()
      try
        if resultSet.next() then Some(decodeJsonRow[SystemMetrics](resultSet.getString(1), DeliveryPersistenceDefaults.MetricsTableName))
        else None
      finally resultSet.close()
    finally statement.close()
  }
...
private def upsertMetricsRow(
  ...
  try
    statement.setString(1, primaryStateKey.raw)
    statement.setString(2, deliveryStateJsonPrinter.print(metrics.asJson))
...
private def tableExists(connection: Connection, tableName: DisplayText): Boolean =
  val metaData = connection.getMetaData
  val resultSet = metaData.getTables(null, null, tableName.raw, Array("TABLE"))
  try resultSet.next()
  finally resultSet.close()

private def decodeJsonRow[A: Decoder](rawJson: String, tableName: DisplayText): A =
  decode[A](rawJson).fold(
    error =>
