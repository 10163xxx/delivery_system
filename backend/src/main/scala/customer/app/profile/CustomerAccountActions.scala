...
private def sanitizedCustomerAddress(
    ...
    sanitizeRequiredText(
      ...
      ValidationMessages.Customer.AddressRequired,
    )
      ...
      updateState { current =>
        for
          ...
          _ <- Either.cond(
            ...
            (),
            ValidationMessages.Customer.AtLeastOneAddressRequired,
          )
          _ <- Either.cond(customer.defaultAddress != address, (), ValidationMessages.Customer.ChangeDefaultAddressBeforeDeleting)
        yield
      ...
      updateState { current =>
        for
          ...
          _ <- Either.cond(
            ...
            (),
            ValidationMessages.Customer.RechargeAmountInvalid,
          )
        yield
          withDerivedData(
            updateCustomerEntry(current, customer.id)(entry =>
              entry.copy(
                metrics = entry.metrics.copy(
                  balanceCents = entry.balanceCents + request.amountCents
                )
              )
            ),
            now(),
