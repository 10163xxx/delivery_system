...
private[delivery] def refreshState(state: DeliveryAppState, currentTime: IsoDateTime): DeliveryAppState =
...
private[delivery] def withDerivedData(state: DeliveryAppState, currentTime: IsoDateTime): DeliveryAppState =
...
private[delivery] def initialRegistrationCoupons(customerId: CustomerId, currentTime: IsoDateTime): List[Coupon] =
