...
private val jsonFilePrinter = Printer.spaces2SortKeys

def loadOrCreateJsonFile[A: Encoder: Decoder](path: Path, seed: => A): A =
...
def writeJsonFile[A: Encoder](path: Path, value: A): Unit =
  ensureParentDirectory(path)

  val tempPath = path.resolveSibling(s".${path.getFileName.toString}.tmp")
  Files.writeString(
    tempPath,
    jsonFilePrinter.print(value.asJson),
