...
// Business note: review projection collections and summary assembly for derived delivery state.
...
private[app] def collectReviewDerivedCollections(
  ...
  val riderOneStars = activeReviewedOrders.flatMap { order =>
    order.riderId.filter(riderId =>
      order.riderRating.contains(DeliveryValidationDefaults.ReviewRatingMin) &&
        isAfterReviewReset(order.updatedAt, latestRiderReviewReset.get(new EntityId(riderId.raw)))
    )
  }
...
private[app] def deriveSystemMetrics(
  ...
  SystemMetrics(
    totalOrders = new EntityCount(state.orders.size),
    activeOrders = activeOrderCount(state.orders),
    resolvedTickets = new EntityCount(state.tickets.count(_.status == TicketStatus.Resolved)),
