...
// Business note: automatic dispatch and income projection rules for derived delivery state.
...
def riderIncomeForOrders(
  ...
  RiderEarningPerOrderCents * new EntityCount(
    orders.count(order =>
...
def minutesBetween(from: IsoDateTime, to: IsoDateTime): DurationDays =
  new DurationDays(java.time.Duration.between(Instant.parse(from.raw), Instant.parse(to.raw)).toMinutes)
